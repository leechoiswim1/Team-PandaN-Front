...
const ModalEditing = ({ history, note, ...rest }) => {
  ...
  const handleEditNote = (e) => {
    ...
    if (noteModifiedInputs.title === "") {
      window.alert("제목을 입력하세요.");
      return;
    }
    if (noteModifiedInputs.content === "") {
      window.alert("할 일에 대한 설명을 추가하세요.");
      return;
    }
    if (noteModifiedInputs.deadline === "") {
    ...
    if (fileList.length > 5) {
      window.alert("최대 5개의 파일까지 업로드 할 수 있습니다.");
      return;
    }
